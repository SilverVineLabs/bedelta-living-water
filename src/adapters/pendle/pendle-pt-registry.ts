...
import type { PTMarketState } from "../../guards/pendle-gmx-cross-guard";
...
export function resolvePendlePtMarketState(
  keyOrAddress: PendlePtMarketKey | string,
  overrides: Partial<PTMarketState> = {},
): { entry: PendlePtRegistryEntry; market: PTMarketState } | null {
  const entry = resolvePendlePtRegistryEntry(keyOrAddress);
  if (!entry) return null;
  logPendlePtRegistryVerification(entry);
  return { entry, market: toPendlePtMarketState(entry, overrides) };
}
