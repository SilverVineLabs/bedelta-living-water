...
|--------|--------------|---------------------|
| **Vitest Baseline** | **135 files / 724 PASS (100% Clean)** | `pnpm test` · security-tier Vitest in [`static-analysis-report.json`](./static-analysis-report.json) |
...
`135 files / 724 PASS (100% Clean)` · `3-Tier Security Matrix: 5/0/0 PASS (Vitest, Forge, Slither, Aderyn, pnpm-audit)` · Wasm `<28kb` / `<60µs` (`<150µs` P99 tail).
...
```bash
pnpm install && pnpm test          # 135 files / 724 PASS (100% Clean)
