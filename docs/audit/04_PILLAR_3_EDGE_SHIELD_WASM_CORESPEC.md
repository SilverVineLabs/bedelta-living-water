...
**Companion fuses:** Sequencer 600s grace · Oracle lag fail-closed (`ORACLE_LAG_DEADLOCK_MS`) · Pgate latency **200 ms** (`PGATE_MAX_LATENCY_MS` · R04) · **Pendle Institutional Shield** (`PENDLE_ORACLE_STALE` · TTL 60s sync oracle).
...
|-----------|------|-------------|
| **Sync Market Oracle** | `pendle-market-oracle-adapter.ts` | ✅ Live · zero-I/O `ingest()` / `resolve()` · TTL **60s** |
