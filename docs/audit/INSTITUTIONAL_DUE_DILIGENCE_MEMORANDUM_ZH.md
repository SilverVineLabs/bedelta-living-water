...
| **基線** | **Vitest SSOT：** **180 test files | 803 PASS Clean** · Wasm 熱路徑 **91.2 KiB gzip** · Shield **p50 ~106 µs** |
...
| **壓力與模擬** | 30D Survival Benchmark + 180 test files | 803 PASS Clean 回歸 | `generate-survival-report.ts` · `pnpm test -- --run` |
...
| 指標 | 鎖定值 | 驗證方式 |
|------|--------|---------|
| **Vitest 回歸** | **180 test files | 803 PASS Clean** | `pnpm test -- --run` |
...
| 框架 | 命令 | 預期 |
|------|------|------|
| **全量 Vitest** | `pnpm test -- --run` | **180 test files | 803 PASS Clean** |
...
| **第三道** | 獨立保證 | 180 test files | 803 PASS Clean · Survival Benchmark · 安全矩陣 · DDIP |
...
| # | 檢查項 | 命令 / 介面 | 通過標準 |
|---|--------|------------|---------|
| 1 | 全量回歸 | `pnpm test -- --run` | 180 test files | 803 PASS Clean |
