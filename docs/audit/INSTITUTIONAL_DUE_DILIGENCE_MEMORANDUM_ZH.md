...
| **壓力與模擬** | 30D Survival Benchmark + 173 test files | 765 PASS Clean 回歸 | `generate-survival-report.ts` · `pnpm test -- --run` |
...
| 指標 | 鎖定值 | 驗證方式 |
|------|--------|---------|
| **Vitest 回歸** | **173 test files | 765 PASS Clean** | `pnpm test -- --run` |
...
| 框架 | 命令 | 預期 |
|------|------|------|
| **全量 Vitest** | `pnpm test -- --run` | **173 test files | 765 PASS Clean** |
...
| **第三道** | 獨立保證 | 173 test files | 765 PASS Clean · Survival Benchmark · 安全矩陣 · DDIP |
