...
| **Wayfinder** | Arbitrum-native AI agent engine — TS SDK `withCitadelShield()` / `verifyAgentIntent()` pre-broadcast hook for on-chain agent intents | ⏳ V1.5 ecosystem target · [`@slivervine/citadel-sdk`](../../src/sdk/decorator.ts) · [Wayfinder docs](https://docs.wayfinder.ai) | `pnpm tsx examples/agent-interceptor-demo.ts` (reference harness) |
| **CrewAI / AutoGen** | Enterprise financial agent frameworks — Robinhood/RWA ingress orchestration via Citadel REST `POST /api/hedge/evaluate` | ⏳ Planned reference harness · Python `BaseTool` pattern from [`langchain-agent-adapter.py`](../../examples/adapters/langchain-agent-adapter.py) | `python examples/adapters/langchain-agent-adapter.py [--trip]` |
...
| **[EIP-7702](https://eips.ethereum.org/EIPS/eip-7702)** | EOA Account Abstraction via `SetCode` — Agent Smart Account upgrade path (execution plane only) | Kernel v4 intent composer · §2.4.5 · [`PILLAR_1_GATEHOUSE_ZERODEV_AA_ANALYSIS.md`](../audit/PILLAR_1_GATEHOUSE_ZERODEV_AA_ANALYSIS.md) | ⏳ V1.5 Post-Grant — Shield/Wasm unchanged |
| **[ERC-7715](https://eips.ethereum.org/EIPS/eip-7715)** | Advanced Wallet Permissions — Gatehouse evolution beyond Kernel v3 session keys | §0.1 · §4.1 · `session-key-gates.ts` | ⏳ Post-Grant Design Spec — adapter swap, no Shield rewrite |
...
| **[Arbitrum Stylus SDK](https://github.com/OffchainLabs/stylus-sdk-rs)** (`0.10.7`) | Stylus WASM Co-Processor Alignment — on-chain soil parity with Edge | [`contracts/stylus-probe/src/lib.rs`](../../contracts/stylus-probe/src/lib.rs) · `SliverVineSoilCoprocessor` · [Stylus docs](https://docs.arbitrum.io/stylus/reference/overview) | ✅ Code-Verified · `cargo test` **5/5 PASS** · Sepolia deploy pending |
...
| **Kernel** | ZeroDev Kernel **v0.3.1** (`ZERODEV_KERNEL_VERSION`) — v4 adapter swap ⏳ Post-Grant (V1.5)（§2.4.2） |
...
| **Wayfinder (Arbitrum-native)** | ⏳ V1.5 ecosystem target · TS SDK `withCitadelShield()` / `verifyAgentIntent()` · [Wayfinder docs](https://docs.wayfinder.ai) · [§0.3](#03-turn-key-agent-ecosystem-adapters) | Sub-ms soil gate · ERC-7579 session clip |
| **CrewAI / AutoGen (enterprise)** | ⏳ Planned reference harness — Robinhood/RWA ingress via Python REST · pattern: [`langchain-agent-adapter.py`](../../examples/adapters/langchain-agent-adapter.py) · [§0.3](#03-turn-key-agent-ecosystem-adapters) | `checkSoilResistance()` · Pillar 2 AML escort boundary |
