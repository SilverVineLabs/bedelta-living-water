...
| **Baseline** | Vitest **168 files \| 742 PASS (100% Clean)** · live **`768 PASS`** on `pnpm test -- --run` · Wasm **p50 ~106 µs** · chaos **255/255** |
...
| **Proof** | **768 PASS** Vitest · **255/255** chaos matrix · **4/4** ZeroDev gate · **5/5** bridge tests |
...
| **Regression proof** | **768 PASS** · `zerodev-aa-gate` **4/4** · bridge **5/5** | Vendor QA snapshots | Audit letters | Partial |
...
| **0:34–0:35** | Logo · URL · QR to `/api/grant-audit` | **BeΔ Living Water · SilverVine Labs** | `pnpm test -- --run` · **768 PASS** |

**End card verification strip (on-screen, 0:34–0:35):**

```text
768 PASS · 255/255 chaos · 4/4 ZeroDev gate · 5/5 bridge · p50 106µs Wasm
```
...
| Full regression | End card | — | `pnpm test -- --run` → **768 PASS** |
...
|--------|----------|----------------------------------|
| Vitest | **168 files \| 742 PASS (100% Clean)** | **174 files \| 768 PASS** |
...
> **Note for evaluators:** Documentation SSOT locks **742 PASS** as the grant regression phrase; the live suite on `v1.0_push_BDLW` runs **768 PASS** (additional component/compliance tests). Both are 100% clean — re-run CLI to confirm on your machine.
