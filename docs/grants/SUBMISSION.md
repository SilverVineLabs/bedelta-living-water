...
- **Native LLM Back-off & Retry Intercepts**: `withCitadelShield` ([`src/sdk/decorator.ts`](../../src/sdk/decorator.ts)) maintains an in-memory **60-second mandatory cooldown** per `agentId` after `SOIL_RESISTANCE_TRIP` or `FAIL_CLOSED` severance — surfacing `[Citadel Back-off] MANDATORY_COOLDOWN_ACTIVE` so agent runtimes halt token-burning infinity retry loops and RPC rate-limit storms.
- **Dynamic Threshold Jitter (±2–5 bps)**: `checkSoilResistance()` injects cryptographic pseudo-random noise into slippage / depth fuse evaluation ([`soil-threshold-jitter.ts`](../../src/services/risk-control-lib/soil-threshold-jitter.ts)) — MEV searchers cannot predict static 50 bps cutoff points off-chain.
- **Non-Semantic Bytecode Predicate Assertions**: Shield validates **Asymmetric Predicate Bytecode Hard Assertions** against ERC-4337 UserOp calldata inside the Wasm core — immune to indirect prompt-injection narratives at the natural-language layer ([Technical Specification §0.1](../architecture/TECHNICAL_SPECIFICATION.md#01-bytecode-predicate-verification-v10--erc-7715--post-grant-design-spec)).
...
* **Test Suite**: **Proposal Baseline: 175 test files | 773 PASS (Current Branch Live: 177 test files | 778 PASS Clean)** — re-run `pnpm test -- --run` to confirm. Full matrix: [Verification Matrix](../VERIFICATION_MATRIX.md).
...
Full derivations: [Technical Specification §3.1](../architecture/TECHNICAL_SPECIFICATION.md#31-microsecond-moats) · [Verification Matrix](../VERIFICATION_MATRIX.md) · [`JUDGE_BRIEF.md`](../../JUDGE_BRIEF.md).
...
| **M-CLI** | Vitest **Proposal Baseline: 175 test files | 773 PASS (Current Branch Live: 177 test files | 778 PASS Clean)** | All | ✅ Delivered |
...
```bash
pnpm install
pnpm test -- --run # Proposal Baseline: 175 test files | 773 PASS (Current Branch Live: 177 test files | 778 PASS Clean)
...
**Regression bar:** Vitest **Proposal Baseline: 175 test files | 773 PASS (Current Branch Live: 177 test files | 778 PASS Clean)** · Forge 60/60 · Cargo Stylus 5/5 · Wasm &lt;28 KiB / &lt;60 µs.
