...
* **Formal Verification**: Invariant lemmas `check_consume_sets_consumed_flag` / `check_replay_must_revert` fully coded in [HalmosGateInvariant.t.sol](../../contracts/test/formal/HalmosGateInvariant.t.sol) — **verifiable via `forge test`** (concrete regression `test_regression_replay_invariant_concrete`). Halmos CLI symbolic exploration is optional nightly tier ([`halmos.json`](../audit/halmos.json)); **not claimed as fully completed** when `exitcode: 1`. · [Technical Specification §3](../architecture/TECHNICAL_SPECIFICATION.md#3-cross-venue-risk-engine--defense-matrix-r01r20).
