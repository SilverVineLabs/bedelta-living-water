...
**物理機制：** 多項式張量在 Rust 內 **單次 SIMD 批次評估**，避免 TS 層多次函式呼叫與 `number` 装箱；配合 [Hot/Cold Path 解耦](./HOT_COLD_PATH_DECOUPLING.md) 的 87.76 KiB lean bundle，確保 L1/L2 cache 駐留不被膨脹依賴破壞。
