...
| **合規邊界** | 雙向路由常見；AML 反向路徑難封鎖 | SliverVine Protocol **單向 escort**（RH → Arb only）· inbound 預設拒絕 |
...
| **資本損失會計** | 橋故障常記為「pending loss」 | SliverVine Protocol 強制 **`lostUsd ≡ 0`**；in-flight 僅標記 `IN_FLIGHT_BRIDGE_CAPITAL` |
...
## 3. SliverVine Protocol 如何利用 ZeroDev Smart Routing
...
**SliverVine Protocol 解法：** 將 **完整執行意圖** 哈希寫入 attestation 既有 `payloadHash` 欄位，**不修改** `SliverVineGate.sol` 的 `ATTESTATION_TYPEHASH` 或 struct layout。
...
Smart Routing 改善的是 **執行路徑與 UX**，不是會計規則。無論資金經 ZeroDev UserOp 還是傳統 Across leg 決策，SliverVine Protocol 的資本標籤語意一致：
